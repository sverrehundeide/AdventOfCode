...
var numberOfTimesAtZero = File.ReadAllLines("input.txt")
    .Sum(instructionString => circularDigitsList.Move(instructionString, digitToCount));
...
internal class DialDigitsList : LinkedList<Digit>
{
    ...
    public int Move(string movementInstructionString, int digitToCount)
    {
        var timesAtDigit = 0;
